...
if (!empty($equipment_id)) { 
    ...
    //角色ID存在，进行更新
        if (!(empty($name) && empty($equipmentcategory_id) && empty($location_id))) {
            //UPDATE `equipment_info` SET `name` = '试验1', `equipmentcategory_id` = '0002', `location_id` = '0002' WHERE `equipment_info`.`equipment_id` = '0002'
            $sql_update = "UPDATE `equipment_info` SET `name` = '$name', `equipmentcategory_id` = '$equipmentcategory_id', `location_id` = '$location_id' WHERE `equipment_info`.`equipment_id` = '$equipment_id'";
