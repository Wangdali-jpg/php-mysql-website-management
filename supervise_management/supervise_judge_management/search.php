...
<!--头部--> 
<div class="header"> <h1>查询监管评价页面</h1> 
</div> 
...
//用户名存在
$sql_search = "select * from supervise_judge_info where supervise_judge_id = '$supervise_judge_id'"; 
...
<tr>
 <td>监管评价ID</td><td>监管评价名</td>
</tr>

<?php
if(!empty($supervise_judge_id)){
    echo "<tr><td>$supervise_judge_id</td><td>$name</td><tr>";
}
else{
...
<table style='text-align:left;border:solid' border="1">	
		<tr><td>监管评价ID</td><td>监管评价名</td></tr>
 <?php
     // $conn = mysqli_connect("localhost", "root", "pwd_1234567", "zhongshiyouvs"); 
     require_once('../../initial_interface/db_config.php');
    $conn = db_connect();
    ...
    $sql = "SELECT * FROM `supervise_judge_info`";
